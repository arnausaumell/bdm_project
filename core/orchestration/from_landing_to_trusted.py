...
def transform_tmdb_movies_released(
    ...
    def _extract_reviews_table(movies_df: DataFrame) -> DataFrame:
        ...
        reviews_df = (
            reviews_expanded.select(
                ...
                .otherwise(lit(None))
                .alias("url"),
            ).filter(col("created_at").isNotNull())  # drop rows with malformed dates
        )
    ...
    def _extract_genres_tables(movies_df: DataFrame) -> Tuple[DataFrame, DataFrame]:
        ...
        genres_expanded = movies_df.select(
            col("tmdb_id").alias("movie_tmdb_id"), explode(col("genres")).alias("genre")
        ).filter(
    ...
    def _extract_languages_tables(movies_df: DataFrame) -> Tuple[DataFrame, DataFrame]:
        """
    ...
    def _extract_keywords_tables(movies_df: DataFrame) -> Tuple[DataFrame, DataFrame]:
        """
...
def transform_tmdb_movies_providers(
    ...
    def _extract_providers_tables(
        ...
        movie_providers_df = movie_providers_df.withColumn(
            "movie_provider_id",
            concat_ws("_", col("tmdb_id"), col("provider_id"), col("provider_type")),
        )
    ...
    target_manager.upsert_to_table(providers_df, "providers", merge_key="provider_id")
    target_manager.upsert_to_table(
...
def transform_trakt_movie_data(
    ...
    cleaned_df = cleaned_df.withColumn("trakt_id", col("trakt_id").cast(IntegerType()))
...
def transform_omdb_ratings(
    ...
    def _extract_imdb_data(omdb_df: DataFrame) -> DataFrame:
        ...
        imdb_df = omdb_df.select(
            ...
            format_number(col("imdb_rating").cast(FloatType()), 2).alias("imdb_rating"),
...
def transform_youtube_stats(
    ...
    # Write tables to trusted zone
    target_manager.upsert_to_table(stats_df, "youtube_statistics", merge_key="tmdb_id")
    target_manager.upsert_to_table(comments_df, "youtube_comments", merge_key="tmdb_id")
